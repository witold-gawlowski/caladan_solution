...
async def generate_requests(queue: Queue):
    print("generate_requests from original file")
    """
            ...
            try:
                ...
                async with rate_limiter.acquire(timeout_ms=remaining_ttl):
                    async with async_timeout.timeout(1.0):
                        ...
                        loop_start_ts = timestamp_ms()
                        async with session.request('GET',
                                                   url,
                                                   data=data) as resp:  # type: aiohttp.ClientResponse
                            loop_times[idx] = timestamp_ms() - loop_start_ts
                            idx = (idx + 1) % 200
                            print("average time", sum(loop_times) / 200)
                            json = await resp.json()
                            if json['status'] == 'OK':
                                logger.info(f"API response: status {resp.status}, resp {json}")
                            else:
                                logger.warning(f"API response: status {resp.status}, resp {json}")
                        rate_limiter.request_times[rate_limiter.curr_idx] = timestamp_ms()
                        rate_limiter.curr_idx = (rate_limiter.curr_idx + 1) % rate_limiter.per_second_rate
